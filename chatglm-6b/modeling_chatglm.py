...
# 用于处理可能出现的NaN和inf值，通过将它们替换为零来确保计算的稳定性
...
# 用于从TensorFlow检查点加载权重到PyTorch模型中。这对于迁移学习和在PyTorch中使用预训练模型非常有用
...
# PrefixEncoder类是一个编码器，用于对输入的前缀进行编码
# 它根据配置使用一个两层的MLP(多层感知器)或者直接进行嵌入，输出维度为(batch_size, prefix_length, 2 * layers * hidden)
...
class RotaryEmbedding(torch.nn.Module):
    ...
    # 这是一个空方法，用于从给定的状态字典加载模型参数。在这个代码段中，它没有实现任何功能
    ...
    # 这个方法应用给定的函数（fn）到缓存的余弦和正弦值上，并调用父类的_apply方法
    def _apply(self, fn):
...
# 这个函数将输入张量x在最后一个维度上分为两半，并将它们交换位置
def rotate_half(x):
...
@torch.jit.script
# 这个函数应用了旋转位置嵌入索引，主要通过cos和sin将位置信息添加到输入张量q和k上
...
def attention_fn(
    ...
    # 包括序列长度sq、批大小b、注意力头数np、每个注意力头的隐藏层大小h
    ...
    """
    # 设置输出张量的大小，即下面得到的原始注意力分数output_size：[b, np, sq, sk]
    ...
    # 现在，沿用之前的output_size的注意力分数张量[b, np, sq, sk]
    ...
    # 且基于这个前提条件，即新的output_size：[b, np, sq, hn]，顺带再提下旧的output_size是[b, np, sq, sk]
    value_layer = value_layer.view(value_layer.size(0), output_size[0] * output_size[1], -1)

    # change view [b, np, sq, sk] -> [b * np, sq, sk]
    # 调整注意力概率：对之前得到的前两个维度做下合并：[b, np, sq, sk] =》[b * np, sq, sk]
    attention_probs = attention_probs.view(output_size[0] * output_size[1], output_size[2], -1)

    # matmul: [b * np, sq, hn]
    # 对上一行得到的attention_probs[b * np, sq, sk]
    # 乘以『原始value_layer但中间两个维度合并了的：[sk, b * np, hn]，做1次转置』，即[b * np, sk, hn]
    # 相当于[b * np, sq, sk] x [b * np, sk, hn]，最终得到[b * np, sq, hn]
    context_layer = torch.bmm(attention_probs, value_layer.transpose(0, 1))

    # change view [b, np, sq, hn]
    # 上行得到context_layer的[b * np, sq, hn]通过上面的新output_size[b, np, sq, hn]调整为
    # 4个维度的[b, np, sq, hn]
...
class SelfAttention(torch.nn.Module):
    ...
    def attention_mask_func(attention_scores, attention_mask):
        ...
        # 对于掩码值为True的位置，将注意力得分设置为-10000.0
        attention_scores.masked_fill_(attention_mask, -10000.0)
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            shift_logits = lm_logits[..., :-1, :].contiguous()  # (batch_size, sequence_length, vocab_size)
            shift_labels = labels[..., 1:].contiguous()  # (batch_size, sequence_length)
            # Flatten the tokens
